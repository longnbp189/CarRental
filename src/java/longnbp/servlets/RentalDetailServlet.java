 ...
 * @author Admin
 */
@WebServlet(name = "RentalDetailServlet", urlPatterns = {"/RentalDetailServlet"})
public class RentalDetailServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String rentId = request.getParameter("txtRentalId").trim();
            String date = request.getParameter("searchDate");
            RentalDetailDAO rdDAO = new RentalDetailDAO();
            List<RentalDetailDTO> list = rdDAO.getRentalDetailDTOs(Integer.parseInt(rentId));
            request.setAttribute("DETAIL_LIST", list);
            request.setAttribute("RENT_ID", rentId);
            request.setAttribute("SEARCH_DATE", date);
        } catch (SQLException ex) {
            log("RentalDetailServlet_SQL " + ex.getMessage());
        } catch (NamingException ex) {
            log("RentalDetailServlet_Naming " + ex.getMessage());
        } finally {
